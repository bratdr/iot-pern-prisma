...
const fetchBusesWithSupir = async (url) => {
  const buses = await fetcher(url);
  const supirRequests = buses.map((bus) =>
    axios.get(`http://tracking.ta-tmj.com/api/v1/bis/${bus.id}`)
  );
  const responses = await Promise.all(supirRequests);
  const supirData = responses.map((response) => response.data.data.supir);
  return buses.map((bus, index) => ({
    ...bus,
    supir: supirData[index],
  }));
};
...
const ManageBuses = () => {
  ...
  const { data, error } = useSWR(
    ...
    fetchBusesWithSupir,
    { dedupingInterval: 10000 } // Optional, to cache the data and avoid redundant fetches
  );

  const setEmergencyStatus = async (nomorPolisi) => {
    try {
      await axios.patch("http://tracking.ta-tmj.com/api/v1/bis/status/set-emergency", {
  ...
  const unsetEmergencyStatus = async (nomorPolisi) => {
    try {
      await axios.patch("http://tracking.ta-tmj.com/api/v1/bis/status/unset-emergency", {
        nomorPolisi: nomorPolisi,
      });
      // Refresh data after the status is updated
      mutate("http://tracking.ta-tmj.com/api/v1/bis");
            ...
            <div className="overflow-x-scroll overflow-y-scroll border-b border-gray-200 shadow sm:overflow-hidden sm:rounded-lg">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                    ...
                    <tr key={bus.id}>
                      ...
                      <td className="whitespace-nowrap px-6 py-4">
                        <div className="text-sm text-gray-900">
                          {bus.supir ? bus.supir.nama : "Loading..."}
                        </div>
                      </td>
                      <td className="whitespace-nowrap px-6 py-4">
                        <div className="text-sm text-gray-900">
                      ...
                      <td className="whitespace-nowrap px-6 py-4">
                        <div className="text-sm text-gray-900">{bus.status}</div>
                      </td>
                      <td className="whitespace-nowrap px-6 py-4">
                        <div className="flex gap-2 text-sm">
                          ...
                          {bus.status === "NORMAL" ? (
                  <button
                    onClick={() => setEmergencyStatus(bus.nomorPolisi)}
                  ...
                  <button
                    onClick={() => unsetEmergencyStatus(bus.nomorPolisi)}
